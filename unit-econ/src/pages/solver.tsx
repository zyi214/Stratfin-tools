...
import { AlertTriangle, CheckCircle2, Target, Dices, Play, RotateCcw } from "lucide-react";
          ...
          <div className="mt-4 pt-4 border-t border-border">
            ...
            {goalType === "investment_payback" && (
              <label className="block">
                ...
                <p className="text-xs text-muted-foreground mt-1">Savings needed for 3-yr payback: ${fmt(investmentAmount / (config.annualVolume * 3))} / unit</p>
              </label>
            )}
          </div>
        </div>
              ...
              <div className="bg-card border border-card-border rounded-lg p-5">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  ...
                  <div><div className="text-muted-foreground text-xs mb-1">Payback Period</div>
                    <div className={`font-bold stat-value ${(result.paybackMonths ?? Infinity) <= 36 ? "text-green-400" : "text-yellow-400"}`}>
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Algebraically solve for any cost driver, or run a probabilistic Monte Carlo simulation across your uncertainty ranges.
        </p>
      </div>
      ...
      {tab === "breakeven" ? <BreakevenSolverTab /> : <MonteCarloTab />}
    </div>
  );
}
